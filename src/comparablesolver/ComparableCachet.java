...
public class ComparableCachet implements IComparableSolver {

	private final static String BINARY_NAME = "cachet-wmc";
	
	private final static String BINARY_PATH = "solvers" + File.separator + BINARY_NAME;
	
	private final static String MEMORY_FLAG = "-c";
	
	private final static String ID = "cachet";
	
	private int memoryLimit;
	
	public ComparableCachet(int memoryLimit) {
	...
	public SolverResult getResult(String output) {
		final Pattern pattern = Pattern.compile("Number of solutions\\s+[0-9]*\\.?[0-9]*[eE]?[+-]?\\d+");
	...
	private String buildCommand(String dimacsPath) {
		return BinaryRunner.MONITORING_PREFIX + BINARY_PATH + " " + dimacsPath + " " +  MEMORY_FLAG + " " + memoryLimit;
	}
